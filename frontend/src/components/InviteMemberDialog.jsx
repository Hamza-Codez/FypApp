...
const CredentialPopup = ({ credentials, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-[60] p-4">
            <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-md p-8 w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-300">
                <div className="text-center mb-8">
                    <div className="size-16 bg-emerald-50 dark:bg-emerald-500/10 rounded-md flex items-center justify-center mx-auto mb-4 border border-emerald-100 dark:border-emerald-500/20">
                ...
                <div className="space-y-4">
                    <div className="space-y-1.5">
                        ...
                        <div className="flex items-center gap-2 p-3 bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-md group hover:border-emerald-500/50 transition-all">
                            ...
                            <button onClick={() => handleCopy(credentials.email, "Email")} className="p-2 hover:bg-white dark:hover:bg-zinc-800 rounded-md text-zinc-400 hover:text-emerald-500 transition-all shadow-sm active:scale-95">
                    ...
                    <div className="space-y-1.5">
                        ...
                        <div className="flex items-center gap-2 p-3 bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-md group hover:border-emerald-500/50 transition-all">
                            ...
                            <button onClick={() => handleCopy(credentials.password, "Password")} className="p-2 hover:bg-white dark:hover:bg-zinc-800 rounded-md text-zinc-400 hover:text-emerald-500 transition-all shadow-sm active:scale-95">
                                <FileText className="size-4" />
                            </button>
                        </div>
                    </div>
                ...
                <div className="mt-8 flex flex-col gap-3">
                    <div className="p-3 bg-amber-50 dark:bg-amber-500/5 border border-amber-100 dark:border-amber-900/20 rounded-md flex gap-3">
                    ...
                    <button onClick={onClose} className="w-full bg-zinc-900 dark:bg-white text-white dark:text-zinc-950 font-black uppercase tracking-widest text-[10px] py-4 rounded-md shadow-xl transition-all active:scale-[0.98]">
                        Done, I've copied it
                    </button>
                </div>
            </div>
...
const CSVResultsPopup = ({ results, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-[60] p-4">
            <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-md p-8 w-full max-w-2xl max-h-[80vh] flex flex-col shadow-2xl animate-in zoom-in-95 duration-300">
                ...
                <div className="flex-1 overflow-y-auto mb-6 border border-zinc-100 dark:border-zinc-900 rounded-md">
                            ...
                            {results.map((res, i) => (
                                <tr key={i}>
                                    ...
                                    <td className="px-4 py-3">
                                        <span className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase ${res.status === 'SUCCESS' ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10' : 'bg-red-50 text-red-600 dark:bg-red-500/10'
                                            }`}>
                                            {res.status}
                                        </span>
                                    </td>
                ...
                <div className="flex gap-3 mt-auto">
                    <button onClick={handleCopyAll} className="flex-1 flex items-center justify-center gap-2 border border-zinc-200 dark:border-zinc-800 py-3 rounded-md font-black uppercase text-[10px] tracking-widest dark:text-white hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-all">
                        <FileText className="size-4" /> Copy All Credentials
                    </button>
                    <button onClick={onClose} className="flex-1 bg-emerald-600 text-white py-3 rounded-md font-black uppercase text-[10px] tracking-widest shadow-lg shadow-emerald-500/20 active:scale-[0.98] transition-all">
                        Close
                    </button>
                </div>
            </div>
...
const InviteMemberDialog = ({ isDialogOpen, setIsDialogOpen, initialData, onSuccess }) => {
    ...
    useEffect(() => {
        if (isDialogOpen && initialData) {
            setFormData({
                ...
                onboard_date: initialData.onboard_date || new Date().toISOString().split('T')[0],
            });
    ...
    const handleSubmit = async (e) => {
        ...
        if (tab === "manual") {
            ...
            // Map custom role to designation and set system role strictly to "EMPLOYEE"
            const customRole = sanitizedData.role || "Employee";
            if (customRole.toUpperCase() === "HR" || customRole.toUpperCase() === "EMPLOYEE") {
                sanitizedData.role = customRole.toUpperCase();
                sanitizedData.designation = customRole;
            } else {
                sanitizedData.designation = customRole;
                sanitizedData.role = "EMPLOYEE";
            }
        ...
        if (addEmployee.fulfilled.match(resultAction) || uploadEmployeesCSV.fulfilled.match(resultAction)) {
            ...
            setFormData({ first_name: "", last_name: "", email: "", gender: "", age: "", role: "Employee", onboard_date: new Date().toISOString().split('T')[0] });
            setCsvFile(null);
        } else {
            const errorDetail = resultAction.payload?.detail;
            const errorMsg = Array.isArray(errorDetail)
                ? errorDetail.map(err => `${err.loc[err.loc.length - 1]}: ${err.msg}`).join(', ')
                : (typeof errorDetail === 'string' ? errorDetail : "Action failed");
    ...
    return (
        <>
            <div className="fixed inset-0 bg-black/10 dark:bg-black/60 backdrop-blur-md flex items-center justify-center z-50 p-4">
                <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-md p-6 w-full max-w-3xl shadow-2xl relative animate-in fade-in zoom-in-95 duration-300">
                    <div className="mb-6 flex items-center justify-between border-b border-zinc-100 dark:border-zinc-900 pb-5">
                        ...
                        <div className="flex bg-zinc-100 dark:bg-zinc-900 p-1 rounded-md w-48 shadow-inner">
                            <button
                                type="button"
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        {tab === "manual" ? (
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-x-4 gap-y-4">
                                {[
                                    ...
                                    <div key={field.name} className="group">
                                        ...
                                        <div className="border border-zinc-200 dark:border-zinc-800 rounded-md p-2.5 bg-zinc-50/50 dark:bg-zinc-900/30 focus-within:border-emerald-500/50 focus-within:ring-2 focus-within:ring-emerald-500/5 transition-all">
                                            <input
                                                name={field.name}
                                                type={field.type}
                                ...
                                <div className="group">
                                    <label className="text-[9px] font-bold uppercase text-zinc-400 tracking-widest block mb-1.5 transition-colors group-focus-within:text-emerald-500">Onboard Date</label>
                                    <div className="border border-zinc-200 dark:border-zinc-800 rounded-md p-2.5 bg-zinc-50/50 dark:bg-zinc-900/30 focus-within:border-emerald-500/50 focus-within:ring-2 focus-within:ring-emerald-500/5 transition-all">
                                        <input
                                            name="onboard_date"
                                            type="date"
                            ...
                            <div className="flex items-center gap-8 py-2 px-1">
                                <div className="flex-1 relative border-2 border-dashed border-zinc-200 dark:border-zinc-800 rounded-md p-8 text-center bg-zinc-50/50 dark:bg-zinc-900/20 hover:border-emerald-400 dark:hover:border-emerald-600 transition-all group overflow-hidden">
                                    ...
                                    <div className="inline-flex px-4 py-1.5 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-md text-[9px] font-black uppercase tracking-widest shadow-sm group-hover:shadow-md transition-all">
                                        {csvFile ? csvFile.name : 'Select File'}
                                    </div>
                                </div>
                                <div className="w-64 space-y-3">
                                    <div className="p-3 bg-emerald-50/50 dark:bg-emerald-900/10 rounded-md border border-emerald-100 dark:border-emerald-900/30">
                                    ...
                                    <div className="p-3 bg-zinc-100/50 dark:bg-zinc-900/50 rounded-md border border-zinc-200 dark:border-zinc-800">
                        ...
                        <div className="flex flex-col sm:flex-row items-center justify-between pt-6 border-t border-zinc-100 dark:border-zinc-900 gap-4">
                            ...
                            <button
                                ...
                                className="w-full sm:w-auto bg-emerald-600 hover:bg-emerald-700 text-white font-black uppercase tracking-widest text-[10px] py-3 px-10 rounded-md shadow-lg shadow-emerald-500/20 active:scale-[0.98] transition-all disabled:opacity-50"
                            >
